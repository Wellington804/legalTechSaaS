...
export function LegalAiWorkbench({ caseId, onSaved }: { caseId?: string; onSaved: () => void }) {
  ...
  return <div className="space-y-4">
    ...
    {matrixResult && <Panel title="2. Revisar e aprovar a base da minuta" description="Marque somente itens sustentados que você conferiu. Conflitos, lacunas e itens sem fonte ficam fora da geração.">
      <p className="text-xs text-zinc-400">Modelo: {matrixResult.provider} / {matrixResult.model}. Cobertura textual aproximada: {matrixResult.coverage.source_characters.toLocaleString("pt-BR")} de {matrixResult.coverage.total_content_characters.toLocaleString("pt-BR")} caracteres.{matrixResult.coverage.truncated ? " A seleção foi limitada; confira os originais." : ""}</p>
