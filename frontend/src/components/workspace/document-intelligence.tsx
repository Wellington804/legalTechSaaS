...
type Analysis = Row & {
  ...
  error?: string;
  sources: Array<{ document_id: string; title: string; version: number; ocr_status?: string; extractor?: string }>;
};

function isPartial(item: Analysis) {
...
export function DocumentIntelligence({ caseId, documents }: { caseId: string; documents: Row[] }) {
  ...
  async function review(item: Analysis, decision: "approve" | "reject") {
    if (decision === "approve" && isPartial(item) && !partialAcknowledged[item.id]) {
  ...
  return <Panel title="Inteligência documental" description="Classifica anexos, monta a linha do tempo probatória e aponta divergências com referência ao documento. Nada é aprovado automaticamente.">
    ...
    <div className="space-y-4">{analyses.data?.map(item => <article key={item.id} className="space-y-3 rounded-xl border border-zinc-800 p-4">
      ...
      <ul className="grid gap-2 text-xs text-zinc-400 sm:grid-cols-2">{item.sources.map(source => <li key={source.document_id} className="rounded-lg border border-zinc-800 p-2">
        {source.title} · {source.ocr_status === "failed" ? "texto incompleto; confira o original" : "texto disponível para análise"}
      </li>)}</ul>
      ...
      {item.coverage && <p className={`text-xs ${isPartial(item) ? "text-amber-300" : "text-zinc-400"}`}>{isPartial(item) ? "A análise usou apenas parte do conteúdo disponível; confira os documentos originais." : "O conteúdo disponível foi analisado, mas os documentos originais ainda exigem conferência."}</p>}
      {item.status === "review_required" && isPartial(item) && <label className="flex min-h-11 items-center gap-3 rounded-lg border border-amber-500/40 p-3 text-sm text-amber-100">
