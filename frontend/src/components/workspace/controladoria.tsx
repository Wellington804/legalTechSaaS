...
type Subscription = { id: string; case_id: string; source_kind: "datajud" | "escavador"; provider_subscription_id: string | null; tribunal: string; process_number: string; status: "active" | "paused" | "disabled"; last_checked_at: string | null; last_success_at: string | null; last_error_code: string | null };
...
const providerLabels = { datajud: "DataJud", escavador: "Escavador" };

function subscriptionStatusText(item: Subscription) {
  if (item.source_kind === "escavador" && item.status === "active" && !item.provider_subscription_id) return "Ativação no provedor pendente";
