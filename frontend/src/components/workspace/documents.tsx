...
export function Documents({ templates = false, caseId, embedded = false }: { templates?: boolean; caseId?: string; embedded?: boolean }) {
  ...
  const [section, setSection] = useState<"files" | "create">("files");
  ...
  const [review, setReview] = useState<{ text: string; stale: boolean; source: { version: number }; provider?: string; model?: string; purpose?: "summary" | "tasks" | "draft" } | null>(null);
