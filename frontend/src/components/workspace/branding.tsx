...
import {
  brandPreflight, brandSettingLabels, defaultBrandSettings, documentTypeLabels, DOCUMENT_TYPES, effectiveBrandSettings,
  ...
  type BrandFontFamily, type DocumentType, type ProfessionalData, type ProfessionalField,
} from "@/lib/branding";
...
function BrandEditor({ initial, initialElement, initialMobileTab, capabilities, onBack, onChanged }: { initial: BrandProfile; initialElement: EditorElement; initialMobileTab: "edit" | "ai"; capabilities: BrandCapabilities; onBack: () => void; onChanged: () => void }) {
  ...
  function deleteLayer(id: string) {
    const layer = settings.layout_layers.find(item => item.id === id);
    if (!layer || layer.locked || busy) return;
    lastHistoryAt.current = 0;
    setLayers(settings.layout_layers.filter(item => item.id !== id)); setSelectedLayerId("");
    setMessage(`Elemento “${layer.label}” excluído. Use Desfazer se precisar restaurá-lo.`);
  }
...
function EditorControls({ element, scope, capabilities, settings, baseSettings, assets, professional, documentType, busy, change, toggleProfessional, kind, setKind, upload, openReference, preview, publish, approved, setApproved, dirty, versions, restoreVersion, preflight, selectedReferenceId, setSelectedReferenceId, referencePages, setReferencePages, crop, setCrop, extractReference, selectedLayerId, setSelectedLayerId, setLayers, deleteLayer, editImage }: {
  ...
  selectedLayerId: string; setSelectedLayerId: (id: string) => void; setLayers: (layers: BrandLayer[]) => void; deleteLayer: (id: string) => void;
  editImage: () => void;
}) {
...
function LayerEditor({ settings, assets, professional, fonts, busy, selectedId, select, setLayers, remove, editImage }: {
  ...
  const moveTo = (edge: "back" | "front") => selected && !selected.locked && setLayers(moveBrandLayerToEdge(layers, selected.id, edge));
  ...
  const range = (key: "width_percent" | "height_percent" | "opacity" | "image_contrast" | "rotation_deg", label: string, min: number, max: number, step = 1, value = selected?.[key]) => selected && <Field label={`${label}: ${key === "opacity" ? `${Math.round(Number(value) * 100)}%` : key === "rotation_deg" ? `${Math.round(Number(value))}°` : key === "image_contrast" ? `${Number(value).toFixed(1)}×` : `${Math.round(Number(value))}%`}`}><input className="w-full accent-blue-600" type="range" min={min} max={max} step={step} value={Number(value)} disabled={busy || selected.locked} onChange={event => update({ [key]: event.target.valueAsNumber })} /></Field>;
  const horizontal = (where: "left" | "center" | "right") => selected && update({ x_percent: where === "left" ? 0 : where === "center" ? (100 - selected.width_percent) / 2 : 100 - selected.width_percent });
  const vertical = (where: "top" | "middle" | "bottom") => selected && update({ y_percent: where === "top" ? 0 : where === "middle" ? (100 - selected.height_percent) / 2 : 100 - selected.height_percent });
  return <div className="space-y-4">
    <p className="text-sm text-zinc-400">Selecione na folha ou na lista. Arraste para posicionar; as alças ajustam o tamanho. Pressione Delete para excluir e use Desfazer se mudar de ideia.</p>
    ...
    {selected && <section className="space-y-3 rounded-xl border border-zinc-700 p-3" aria-label={`Configurar ${selected.label}`}>
      <div className="flex items-center justify-between gap-2"><strong className="text-sm">{selected.label}</strong><button type="button" className={button} aria-label="Duplicar elemento" disabled={busy} onClick={duplicate}><Copy aria-hidden="true" size={15} /> Duplicar</button></div>
      <button type="button" className={`${button} w-full border-rose-800 text-rose-300 hover:bg-rose-950/30`} disabled={busy || selected.locked} onClick={() => remove(selected.id)}><Trash2 aria-hidden="true" size={16} /> Excluir elemento</button>
      ...
      <div className="space-y-3">{range("width_percent", "Largura", 1, 100)}{range("height_percent", "Altura", 1, 100)}{range("opacity", "Visibilidade", 0, 1, .01)}{range("rotation_deg", "Giro", -180, 180)}{selected.kind === "image" && range("image_contrast", "Contraste da imagem", .5, 3, .05)}</div>
      <details className="rounded-lg border border-zinc-800 p-3"><summary className="cursor-pointer text-sm font-medium">Ajuste preciso</summary><p className="mt-2 text-xs text-zinc-400">Use estes valores somente quando precisar repetir uma medida exata.</p><div className="mt-3 grid grid-cols-2 gap-2">{numeric("x_percent", "Distância da esquerda (%)", 0, 100)}{numeric("y_percent", "Distância do topo (%)", 0, 100)}{numeric("width_percent", "Largura (%)", 1, 100)}{numeric("height_percent", "Altura (%)", 1, 100)}{numeric("opacity", "Visibilidade (0 a 1)", 0, 1, .01)}{numeric("rotation_deg", "Giro (graus)", -180, 180)}{selected.kind === "image" && numeric("image_contrast", "Contraste", .5, 3, .05)}</div></details>
      ...
      {(selected.kind === "text" || selected.kind === "icon_text") && <><div className="grid grid-cols-2 gap-2">{numeric("font_size_pt", "Tamanho (pt)", 5, 40, .5)}{numeric("letter_spacing_pt", "Espaçamento", 0, 5, .1)}</div><Field label="Fonte"><select className={control} value={selected.font_family} disabled={busy} onChange={event => update({ font_family: event.target.value as BrandLayer["font_family"] })}>{fonts.map(font => <option key={font}>{font}</option>)}</select></Field></>}
