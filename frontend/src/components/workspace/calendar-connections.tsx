...
export function CalendarConnections({ tasks }: { tasks: TaskChoice[] }) {
  ...
  return <section className="space-y-3 border-t border-zinc-800 pt-4"><div><h3 className="font-medium">Sincronização bidirecional</h3><p className="mt-1 text-sm text-zinc-400">Só os compromissos escolhidos são enviados. Edições simultâneas viram conflito para revisão; exclusões externas nunca apagam prazo jurídico automaticamente.</p></div><aside className="rounded-lg border border-zinc-800 bg-zinc-950/30 p-3 text-xs leading-relaxed text-zinc-400"><span className="font-medium text-zinc-300">Dados enviados ao provedor:</span> título, data e hora, duração de uma hora, local, observações e identificadores técnicos do compromisso e da conexão. O LexFlow lê alterações apenas da agenda escolhida e persiste somente eventos vinculados.</aside><State loading={connections.loading} error={connections.error} /><div className="grid gap-3 lg:grid-cols-2">{(["google", "microsoft"] as Provider[]).map(provider => <ConnectionCard key={provider} provider={provider} connection={connections.data?.items.find(item => item.provider === provider)} tasks={tasks} />)}</div>{Boolean(conflicts.data?.items.length) && <div className="space-y-3 rounded-lg border border-amber-800 bg-amber-950/15 p-3"><h3 className="font-medium text-amber-200">Revisão de alterações externas</h3>{conflicts.data?.items.map(conflict => <article key={conflict.id} className="space-y-3 border-t border-amber-900/60 pt-3"><p className="text-sm">{tasks.find(task => task.id === conflict.task_id)?.title || "Compromisso"}: {conflict.reason === "remote_deleted" ? "foi excluído na agenda externa" : "mudou no LexFlow e na agenda externa"}. Compare os dois lados antes de decidir.</p><div className="grid gap-2 md:grid-cols-2"><ConflictSnapshot title="Versão no LexFlow" side={conflict.local} /><ConflictSnapshot title="Versão na agenda externa" side={conflict.remote} /></div><div className="flex flex-wrap gap-2"><Action className={primary} run={() => api.post(`/integrations/calendar-oauth/conflicts/${conflict.id}/resolve`, { resolution: "keep_local", expected_local_revision: conflict.local.revision ?? conflict.local_revision, expected_remote_hash: conflict.remote_hash })} onDone={conflicts.reload}>Manter LexFlow</Action><Action className={button} run={() => api.post(`/integrations/calendar-oauth/conflicts/${conflict.id}/resolve`, { resolution: "accept_remote", expected_local_revision: conflict.local.revision ?? conflict.local_revision, expected_remote_hash: conflict.remote_hash })} onDone={conflicts.reload}>Aceitar agenda externa</Action></div></article>)}</div>}<p className="text-xs text-zinc-500">No iPhone ou iPad, mantenha a mesma conta Google ou Microsoft adicionada em Ajustes → Apps → Calendário → Contas. Não é necessário informar senha do iCloud nem configurar CalDAV no LexFlow.</p></section>;
}
