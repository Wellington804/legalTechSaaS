...
import { Bot, CheckSquare, Copy, FilePlus2, History, Mic, Paperclip, Pencil, Plus, RotateCcw, Search, Send, Square, Trash2, X } from "lucide-react";
...
type Source = { kind: string; id: string; label: string; url?: string };
...
const starters = ["Organize minhas prioridades sem inventar prazos.", "Revise este texto e separe fatos, lacunas e riscos.", "Prepare um roteiro de atendimento para o cliente.", "Compare os documentos anexados e destaque divergências."];
...
function StructuredAnswer({ text }: { text: string }) {
  ...
  return <div className="space-y-3">{parts.map((part, index) => {
    ...
    return <section key={`${title}:${index}`} className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-3"><h3 className="text-sm font-semibold text-zinc-100">{title}</h3><p className="mt-1 whitespace-pre-wrap text-sm leading-relaxed text-zinc-300">{body.join("\n").trim()}</p></section>;
  })}</div>;
}
...
export function AssistantWorkspace() {
  ...
  const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [notice, setNotice] = useState(""); const [historyOpen, setHistoryOpen] = useState(false);
  ...
  return <Page title="Copiloto jurídico" subtitle="Converse com contexto do escritório, analise documentos e transforme respostas em rascunhos ou tarefas sempre sob sua confirmação.">
    <div className="grid min-h-[70dvh] overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/20 lg:grid-cols-[18rem_minmax(0,1fr)]">
      <aside className={`${historyOpen ? "fixed inset-0 z-50 flex" : "hidden"} flex-col border-r border-zinc-800 bg-zinc-950 lg:static lg:flex`} aria-label="Histórico pessoal">
        ...
        <div className="min-h-0 flex-1 space-y-1 overflow-y-auto p-2">{conversations.map(item => <div key={item.id} className={`group rounded-xl p-2 ${item.id === conversationId ? "bg-blue-500/15" : "hover:bg-zinc-900"}`}><button className="w-full text-left" onClick={() => void openConversation(item.id)}><span className="block truncate text-sm font-medium">{item.title}</span><span className="text-xs text-zinc-500">{item.message_count} mensagens · {new Date(item.updated_at).toLocaleDateString("pt-BR")}</span></button><div className="mt-1 flex gap-1"><button className="min-h-9 min-w-9 rounded-lg text-zinc-400 hover:bg-zinc-800" onClick={() => void renameConversation(item)} aria-label={`Renomear ${item.title}`}><Pencil className="mx-auto" size={14} /></button><button className="min-h-9 min-w-9 rounded-lg text-zinc-400 hover:bg-zinc-800" onClick={() => void deleteConversation(item)} aria-label={`Excluir ${item.title}`}><Trash2 className="mx-auto" size={14} /></button></div></div>)}{conversations.length === 0 && <p className="p-3 text-sm text-zinc-500">Nenhuma conversa salva.</p>}</div>
      </aside>
      ...
      <section className="flex min-w-0 flex-col">
        ...
        <div className="grid gap-2 border-b border-zinc-800 p-3 sm:grid-cols-3">
          <Field label="Contexto"><select className={control} value={kind} onChange={event => { const value = event.target.value as ContextKind; setKind(value); setClientId(""); setCaseId(""); setDocumentId(""); }}><option value="global">@Geral</option><option value="client">@Cliente</option><option value="case">@Processo</option><option value="document">@Documento</option><option value="library">@Biblioteca</option><option value="branding">@Identidade</option></select></Field>
          {kind === "client" ? <Field label="Cliente"><select className={control} value={clientId} onChange={event => setClientId(event.target.value)}><option value="">Selecione…</option>{clients.data?.items.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select></Field> : kind === "case" ? <Field label="Processo"><select className={control} value={caseId} onChange={event => setCaseId(event.target.value)}><option value="">Selecione…</option>{cases.data?.items.map(row => <option key={row.id} value={row.id}>{row.title}</option>)}</select></Field> : kind === "document" ? <Field label="Documento"><select className={control} value={documentId} onChange={event => setDocumentId(event.target.value)}><option value="">Selecione…</option>{documents.data?.items.map(row => <option key={row.id} value={row.id}>{row.title}</option>)}</select></Field> : <div className="self-end rounded-xl border border-zinc-800 p-3 text-sm text-zinc-400">{contextNames[kind]}</div>}
          <Field label="Guardar histórico"><select className={control} value={retention} onChange={event => void changeRetention(Number(event.target.value) as 30 | 90 | 365)}><option value={30}>30 dias</option><option value={90}>90 dias</option><option value={365}>1 ano</option></select></Field>
        </div>

        <div ref={log} role="log" aria-live="polite" className="min-h-[24rem] flex-1 space-y-5 overflow-y-auto p-4 md:p-6">
          {messages.length === 0 && <div className="mx-auto max-w-2xl space-y-5 py-8 text-center"><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-blue-500/15 text-blue-300"><Bot size={26} /></span><div><h2 className="text-xl font-semibold">Como posso ajudar no trabalho de hoje?</h2><p className="mt-2 text-sm text-zinc-400">Escolha um contexto, anexe documentos ou faça uma pergunta. Nada será enviado, protocolado ou transformado em prazo automaticamente.</p></div><div className="grid gap-2 text-left sm:grid-cols-2">{starters.map(item => <button key={item} className={`${button} justify-start`} onClick={() => setQuestion(item)}>{item}</button>)}</div></div>}
          {messages.map(message => <article key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[94%] md:max-w-[85%] ${message.role === "user" ? "rounded-2xl bg-blue-600 px-4 py-3 text-white" : "w-full space-y-3"}`}>{message.role === "assistant" ? <><StructuredAnswer text={message.text} />{message.sources && message.sources.length > 0 && <details className="rounded-xl border border-zinc-800 p-3"><summary className="cursor-pointer text-sm font-medium">Fontes e registros usados ({message.sources.length})</summary><ul className="mt-2 space-y-1 text-xs text-zinc-400">{message.sources.map((source, index) => <li key={`${source.kind}:${source.id}:${index}`}>{source.label}</li>)}</ul></details>}</> : <><p className="whitespace-pre-wrap text-sm">{message.text}</p>{message.attachments && message.attachments.length > 0 && <p className="mt-2 text-xs text-blue-100">{message.attachments.map(item => item.label).join(" · ")}</p>}<button className="mt-2 text-xs underline" onClick={() => setQuestion(message.text)}>Editar e reenviar</button></>}</div></article>)}
