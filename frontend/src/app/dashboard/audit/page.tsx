...
export default function PageView() {
  ...
  return <Page title="Histórico do escritório" subtitle="Veja quem alterou informações e quando.">
    ...
    <Panel title="Alterações recentes"><State loading={logs.loading} error={logs.error || members.error} />{logs.data && !logs.data.length && <p className="text-sm text-zinc-400">Nenhuma alteração corresponde aos filtros.</p>}<div className="space-y-2">{logs.data?.map(log => <article key={log.id} className="rounded-lg border border-zinc-800 bg-zinc-950/30 p-4"><p className="text-sm font-medium">{log.actor_name} {actions[log.action] || "realizou uma alteração"}.</p><p className="mt-1 text-xs text-zinc-400">{dateText(log.created_at)} · {areas[log.resource_type] || "Administração"}</p></article>)}</div></Panel>
  </Page>;
}
