...
class CalendarClient:
    ...
    async def _request(self, method: str, url: str, *, json_body: dict | None = None, extra_headers: dict | None = None) -> dict:
        parsed = urlsplit(url)
    ...
    async def calendars(self) -> tuple[ProviderCalendar, ...]:
        ...
        return tuple(
            ProviderCalendar(str(row["id"]), str(row.get("name") or row["id"]), bool(row.get("isDefaultCalendar")), bool(row.get("canEdit", True)))
            for row in rows if isinstance(row, dict) and row.get("id")
        )
    ...
    async def create_event(self, calendar_id: str, task_id: str, body: dict[str, Any], connection_id: str) -> RemoteEvent:
        if self.provider == "google":
            ...
            payload = await self._request(
                ...
                json_body={**_microsoft_body(body, task_id), "transactionId": hashlib.sha256(f"{connection_id}:{task_id}".encode()).hexdigest()},
            )
        return _parse_event(self.provider, payload)

    async def update_event(self, calendar_id: str, event_id: str, etag: str | None, task_id: str, body: dict[str, Any]) -> RemoteEvent:
        url = (
        ...
        payload = await self._request(
            "PATCH", url, json_body=_google_body(body) if self.provider == "google" else _microsoft_body(body, task_id),
    ...
    async def delete_event(self, calendar_id: str, event_id: str, etag: str | None) -> None:
        ...
        await self._request("DELETE", url, extra_headers={"If-Match": etag} if etag else None)
    ...
    async def changes(self, calendar_id: str, connection_id: str, cursor: str | None, start: datetime, end: datetime) -> RemoteChangePage:
        ...
        if cursor:
            ...
            query = urlencode({"showDeleted": "true", "singleEvents": "true"})
...
def _microsoft_body(body: dict[str, Any], task_id: str) -> dict[str, Any]:
    start = body["starts_at"]
    ...
    return {
        ...
        "body": {"contentType": "text", "content": f"{note}\n\nLexFlow-Task-ID: {task_id}".strip()},
        "categories": ["LexFlow"],
    }
...
def _parse_event(provider: str, row: dict[str, Any]) -> RemoteEvent:
    ...
    if provider == "google":
        ...
        if isinstance(notes, str):
            notes = notes.split("\n\nLexFlow-Task-ID:", 1)[0].strip()
        etag = row.get("@odata.etag") or row.get("changeKey")
    return RemoteEvent(str(row["id"]), str(etag) if etag else None, deleted, title if isinstance(title, str) else None, _parse_datetime(start) if start and not deleted else None, location if isinstance(location, str) else None, notes if isinstance(notes, str) else None)
