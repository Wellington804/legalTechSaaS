"""Durable asynchronous finalization for Autentique and Clicksign artifacts."""
...
from app.models.operations import SignatureProviderEvent
from app.services.operations import finalize_queued_autentique_event, finalize_queued_clicksign_event
...
async def _process_clicksign(event_id: str, tenant_id: str) -> str:
    try:
        async with AsyncSessionLocal() as db:
            await _set_tenant_context(db, tenant_id)
            event_provider = await db.scalar(
                select(SignatureProviderEvent.provider).where(
                    SignatureProviderEvent.tenant_id == tenant_id,
                    SignatureProviderEvent.id == event_id,
                )
            )
            usage_attempt = str(uuid.uuid4())
            result = await finalize_queued_clicksign_event(db, tenant_id=tenant_id, event_id=event_id)
            if result in {"finalized", "provider_deferred", "validation_failed", "validation_deferred"}:
                await record_provider_usage(
                    db,
                    tenant_id=tenant_id,
                    provider=event_provider or "clicksign",
                    metric="document_query",
                    idempotency_key=f"event:{event_id}:attempt:{usage_attempt}",
                )
            await db.commit()
...
@celery_app.task(
    name="clicksign.process_signature_event",
    acks_late=True,
    reject_on_worker_lost=True,
    soft_time_limit=50,
    time_limit=60,
)
def process_clicksign_signature_event(event_id: str, tenant_id: str):
    try:
        return {"status": asyncio.run(_process_clicksign(event_id, tenant_id))}
    except Exception:
        logger.warning("Clicksign artifact revalidation deferred")
        return {"status": "deferred"}
...
async def _clicksign_candidates() -> list[tuple[str, str]]:
    try:
        async with AsyncSessionLocal() as db:
            rows = await db.execute(
                text("SELECT * FROM public.clicksign_signature_event_candidates(:limit)"),
                {"limit": 200},
            )
            return [(row.event_id, row.tenant_id) for row in rows]
    finally:
...
def reconcile_autentique_signed_artifacts():
    ...
    clicksign_candidates = asyncio.run(_clicksign_candidates())
    published = 0
    ...
    for event_id, tenant_id in clicksign_candidates:
        try:
            process_clicksign_signature_event.delay(event_id, tenant_id)
            published += 1
        except Exception:
            logger.warning("Clicksign event publication deferred")
    return {
        "candidates": len(candidates) + len(clicksign_candidates),
        "published": published,
        "autentique_candidates": len(candidates),
        "clicksign_candidates": len(clicksign_candidates),
    }
