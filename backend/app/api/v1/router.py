...
from app.api.v1.endpoints import account, audit, auth, branding, calendar_sync, controladoria, crm, document_kit, engagement, integrations, jurimetria, notifications, oab, operations, pilot, provider_costs, push, research, routines, workspace
...
api_router.include_router(crm.router, prefix="/crm", tags=["Relacionamento e oportunidades"], dependencies=[Depends(require_privileged_mfa)])
api_router.include_router(oab.router, prefix="/oab", tags=["Acompanhamento de inscrição OAB"], dependencies=[Depends(require_privileged_mfa)])
api_router.include_router(jurimetria.router, prefix="/jurimetria", tags=["Jurimetria descritiva"], dependencies=[Depends(require_privileged_mfa)])
